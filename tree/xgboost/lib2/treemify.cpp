...
void preSort_x(Matrix* data, int feaSize,
               map<int, vector<Block>*>* sortIdMap,
               map<int, Block*>* p_sortIdMap,
               map<int, int*>* p_sort2IdMap
               ) {
...
static PyObject * tree_xgboost_gbm(PyObject *self, PyObject *args) {
    ...
    // 解析数据
    Matrix* data = new Matrix(data_raw, trainConf.rowWidth, trainConf.colWidth);

    printf("check data:%f\n", data->in(trainConf.rowWidth - 1, trainConf.colWidth - 1)); 

    vector<float>* predVec = new vector<float>(trainConf.rowWidth, 0.5f);
    vector<float> innerPreds(trainConf.dataSize, 0.0f);

    vector<float>* gradVec = new vector<float>(trainConf.rowWidth, 0.0f);
    vector<float>* hessVec = new vector<float>(trainConf.rowWidth, 0.0f);

    vector<float>* labelVec = new vector<float>(trainConf.rowWidth);
    for (size_t i = 0; i < trainConf.rowWidth; ++i) {
        labelVec->at(i) = data->at(i, trainConf.colWidth - 1);
    }

    map<int, vector<Block>*>* sortIdMap_p = new map<int, vector<Block>*>();
    map<int, Block*>* sortIdMap = new map<int, Block*>();
    map<int, int*>* sort2IdMap = new map<int, int*>();
    preSort_x(data, trainConf.dimSize, sortIdMap_p, sortIdMap, sort2IdMap);
    ...
    for (int i = 0; i < trainConf.treeSize; ++i) {
        ...
        // 采样
        vector<int>* itemVec = new vector<int>();
        rowSample(trainConf.rowSample, trainConf.dataSize, *itemVec);
        vector<int>* feaVec = new vector<int>();
        columnSample(trainConf.colSample, trainConf.dimSize, *feaVec);

        int feaSize = 0;
        for (int _i = 0; _i < feaVec->size(); ++_i) {
            feaSize += feaVec->at(_i);
        }
        printf("use feaSize:%d\n", feaSize);

        float eps = 1e-16f;
        for (size_t j = 0; j < trainConf.dataSize; ++j) {
            float _pred = predVec->at(j);
            float _label = labelVec->at(j);
            _pred = sigmoid(_pred);
            gradVec->at(j) = _pred - _label;
            hessVec->at(j) = std::max(_pred * (1.0f - _pred), eps);
            // gradVec->at(j) = _pred - _label;
            // hessVec->at(j) = 1.0f;
        }

        // 建树
        TreeNode* root = _xgboost_main(data, sortIdMap, sort2IdMap, feaVec, itemVec, predVec, labelVec, gradVec, hessVec);
        printf("xgb_main:%lf\n", getTime() - begin);
        ...
        // set residual error
        double pred_begin = getTime();
        parallel_predict(data, root, 6, dim, trainConf.shrink, innerPreds, *predVec);
        printf("predict:%lf\n", getTime() - pred_begin);

        double _begin = getTime();
        parallel_refresh(data, sortIdMap, dim, 6, *predVec);
