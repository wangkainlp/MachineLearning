...
void* thread_fea(void* param_ptr) {
    ...
    if (end > param->feaVec->size() - 1) {
        end = param->feaVec->size() - 1;
    }
...
static void findBestSpV2(Matrix& data, map<int, vector<int> >& sortIdMap,
    ...
    for (int i = 0; i < threadSize; ++i) { 
        int ret = pthread_create(&thread_pool[i], &attr, thread_fea, (void*)&params[i]);
        if (ret) {
