...
static void findBestSpV2_(Matrix* data, map<int, vector<int> >* sortIdMap,
...
TreeNode* constructTreeNode(vector<int>& itemVec, Matrix* data) {
    TreeNode* newNode = new TreeNode();
    newNode->father = NULL;             /// 
    newNode->size = itemVec.size();
    newNode->depth = 0;                 ///
...
void* threadNode(void* params_ptr) {
    ...
    for (int i = 0; i < node->size; ++i) {
        itemVec.push_back(node->itemIds[i]);
    }
    vector<int> feaVec;
    ...
    findBestSpV2_(param->data, param->sortIdMap, itemVec, feaVec, minFea, minSp, minVar);
    ...
    for (int i = 0; i < itemVec.size(); ++i) {
        if (param->data->at(itemVec[i], minFea) <= minSp) {
            leftItems.push_back(itemVec[i]);
        } else {
            rightItems.push_back(itemVec[i]);
        }
    ...
    TreeNode* leftNode = constructTreeNode(leftItems, param->data);
    TreeNode* rightNode = constructTreeNode(rightItems, param->data);
...
static TreeNode* cart_main(Matrix& data, vector<int>& itemVec, vector<int>& feaVec) {
    ...
    */
    ...
    TreeNode* root = constructTreeNode(itemVec, &data);
    root->depth = 0;
