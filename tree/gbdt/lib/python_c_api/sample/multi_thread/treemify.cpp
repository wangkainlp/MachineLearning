...
void* thread_fea(void* param_ptr) {
    ...
    for (int j = 1; j < itemSize; ++j) {
        if (feaDataVec[j - 1] + 1e-7 < feaDataVec[j]) {
            spList.push_back(0.5 * (feaDataVec[j - 1] + feaDataVec[j]));
        }
    }

    if (spList.size() <= 0) {
        printf("fea:%d spList is none\n", fea);
        continue;
    }
...
static void findBestSpV2(Matrix& data, map<int, vector<int> >& sortIdMap,
    ...
    struct Param params[thread_size];
    ...
    for (int i = 0; i < feaVec.size(); i += thread_size) {
        ...
        for (int j = 0; j < thread_size; ++j) {
            param[i + j].fea = feaVec[i + j];
            int ret = pthread_create(&thread_pool[j], NULL, thread_fea, (void*)&params[i + j]);
            if (ret) {
